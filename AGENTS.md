...
The S1–S6 roadmap and clinical expansions are delivered. Current pending gaps are: full account deletion (LGPD) is not implemented (data export in `.zip` is delivered in `/contas/exportar-dados/`); the Mercado Pago gateway has never run against a real seller account; there is no reference food catalogue (TACO), no automatic workout reminder, and the production WhatsApp channel requires a connected chip. See the "O que ainda não existe" section of `CLAUDE.md`.
