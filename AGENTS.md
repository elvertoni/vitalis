...
Bump the `CACHE` version when changing it. The install banner lives in `templates/base.html` inside `{% block install_prompt %}` (pages with a composer pinned to the bottom, like the chat, empty it)
